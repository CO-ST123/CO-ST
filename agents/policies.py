...
from att import model


class ACPolicy:
    def __init__(self, n_a, n_a_p, n_a_s, n_s, n_step, policy_name, agent_name):
    ...
    def prepare_loss(self, v_coef, max_grad_norm, alpha, epsilon):
        ...
        self.A_p = tf.placeholder(tf.int32, [self.n_step])
        self.A_s = tf.placeholder(tf.int32, [self.n_step])
        ...
        A_sparse_p = tf.one_hot(self.A_p, self.n_a_p)
        A_sparse_s = tf.one_hot(self.A_s, self.n_a_s)
        A_sparse = tf.one_hot(120,15)
        log_pi = tf.log(tf.clip_by_value(self.pi, 1e-10, 1.0))
        log_pi_p = log_pi[:,0:5]
        log_pi_s = log_pi[:,5:16]
        ...
        policy_loss_p = -tf.reduce_mean(tf.reduce_sum(log_pi_p * A_sparse_p, axis=1) * self.ADV)
        policy_loss_s = -tf.reduce_mean(tf.reduce_sum(log_pi_s * A_sparse_s, axis=1) * self.ADV)
        value_loss = tf.reduce_mean(tf.square(self.R - self.v)) * 0.5 * v_coef
        self.loss = policy_loss_p + policy_loss_s + value_loss + entropy_loss
...
class LstmACPolicy(ACPolicy):
    def __init__(self, n_s, n_a, n_a_p, n_a_s, n_w, n_step, n_fc_wave=128, n_fc_wait=32, n_lstm=64, name=None):
        super().__init__(n_a, n_a_p, n_a_s, n_s, n_step, 'lstm', name)
        self.n_lstm = n_lstm
        self.n_fc_wait = n_fc_wait
        self.n_fc_wave = n_fc_wave
        self.n_w = n_w
        self.ob_fw = tf.placeholder(tf.float32, [1, n_s + n_w])
        self.done_fw = tf.placeholder(tf.float32, [1])
        self.ob_bw = tf.placeholder(tf.float32, [n_step, n_s + n_w])
...
class FPLstmACPolicy(LstmACPolicy):
    def __init__(self, n_s, n_a, n_a_p, n_a_s, n_w, n_f, n_step, n_fc_wave=128, n_fc_wait=32, n_fc_fp=32, n_lstm=64, name=None):
        ACPolicy.__init__(self, n_a, n_a_p, n_a_s, n_s, n_step, 'fplstm', name)
        self.n_lstm = n_lstm
        self.n_fc_wave = n_fc_wave
        self.n_fc_wait = n_fc_wait
        ...
        self.states = tf.placeholder(tf.float32, [2, n_lstm * 2])
        with tf.variable_scope(self.name):
            self.pi_fw, pi_state = self._build_net_pi('forward', 'pi')
            self.v_fw, v_state = self._build_net_v('forward', 'v')
            pi_state = tf.expand_dims(pi_state, 0)
            v_state = tf.expand_dims(v_state, 0)
            self.new_states = tf.concat([pi_state, v_state], 0)
        with tf.variable_scope(self.name, reuse=True):
            self.pi, _ = self._build_net_pi('backward', 'pi')
            self.v, _ = self._build_net_v('backward', 'v')
        self._reset()

    def _build_net_pi(self, in_type, out_type):
        if in_type == 'forward':
            ob = self.ob_fw
            done = self.done_fw
        else:
        ...
        h = tf.convert_to_tensor(ob, dtype=float)
    ...
    def _build_net_v(self, in_type, out_type):
        if in_type == 'forward':
            ob = self.ob_fw
            done = self.done_fw
        else:
        ...
        D = d * K
        bn = True
        X = tf.convert_to_tensor(ob, dtype=float)
        X1 = tf.expand_dims(X, axis=0)
        X2 = tf.expand_dims(X1, axis=0)
        X3 = model.FC(
            X2, units=[D, D], activations=[tf.nn.relu, None],
            bn=bn, bn_decay=bn_decay, is_training=is_training)

        attention = model.transformAttention(X3, K, d, bn, bn_decay, is_training)

        att_value = tf.squeeze(attention, axis=-1)
        att_value = tf.squeeze(att_value, axis=-1)

        new_states = tf.matmul(att_value, X)

        new_states = tf.nn.softmax(new_states, axis=-1)

        h = tf.reshape(new_states, [1, -1])
