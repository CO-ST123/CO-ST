...
def attention(xs, dones, s, scope, init_scale=DEFAULT_SCALE, init_mode=DEFAULT_MODE,
         init_method=DEFAULT_METHOD):
    xs = batch_to_seq(xs)
    dones = batch_to_seq(dones)
    n_in = xs[0].shape[1].value
...
class OnPolicyBuffer(TransBuffer):
    ...
    def add_transition(self, ob, a_p, a_s, r, v, done):
        self.obs.append(ob)
        self.acts.append(a_p)
        self.acts.append(a_s)
        self.rs.append(r)
        self.vs.append(v)
