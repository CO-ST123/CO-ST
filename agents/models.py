...
class A2C:
    def __init__(self, n_s, n_i_s, n_a, n_a_p, n_a_s, total_step, model_config, seed=0, n_f=None):
        self.name = 'a2c'
        self.n_agent = 1
        ...
        self.policy = self._init_policy(n_s, n_i_s, n_a, n_a_p, n_a_s, n_f, model_config)
    ...
    def _init_policy(self, n_i_s, n_a, n_a_p, n_a_s, n_w, n_f, model_config, agent_name=None):
        ...
        if self.name == 'ma2c':
            ...
            policy = FPLstmACPolicy(n_i_s, n_a, n_a_p, n_a_s, n_w, n_f, self.n_step, n_fc_wave=n_fw,
                                    n_fc_wait=n_ft, n_fc_fp=n_fp, n_lstm=n_lstm, name=agent_name)
        else:
            policy = LstmACPolicy(n_i_s, n_a, n_w, self.n_step, n_fc_wave=n_fw,
    ...
    def add_transition(self, ob, action_p, action_s, reward, value, done):
        if self.reward_norm:
            reward /= self.reward_norm
        if self.reward_clip:
            reward = np.clip(reward, -self.reward_clip, self.reward_clip)
        self.trans_buffer.add_transition(ob, action_p, action_s, reward, value, done)
...
class IA2C(A2C):
    ...
    def add_transition(self, obs, action_p, action_s, rewards, values, done):
        if (self.reward_norm):
            rewards = rewards / self.reward_norm
        if self.reward_clip:
            rewards = np.clip(rewards, -self.reward_clip, self.reward_clip)
        for i in range(self.n_agent):
            self.trans_buffer_ls[i].add_transition(obs[i], action_p[i], action_s[i],
                                                   rewards[i], values[i], done)


class MA2C(IA2C):
    def __init__(self, n_s_ls, n_a_ls, n_a_lsp, n_a_lss, n_w_ls, n_f_ls, total_step,
                 model_config, seed=0):
        self.name = 'ma2c'
        self.agents = []
        self.n_agent = len(n_s_ls)
        ...
        self.n_s_ls = n_s_ls
        self.n_a_ls = n_a_ls
        self.n_a_lsp = n_a_lsp
        self.n_a_lss = n_a_lss
        self.n_f_ls = n_f_ls
        self.n_w_ls = n_w_ls
        self.n_step = model_config.getint('batch_size')
        tf.reset_default_graph()
        ...
        self.sess = tf.Session(config=config)
        self.policy_ls = []
        for i, (n_s, n_a, n_a_p, n_a_s, n_w, n_f) in enumerate(zip(self.n_s_ls, self.n_a_ls, self.n_a_lsp, self.n_a_lss, self.n_w_ls, self.n_f_ls)):
            self.policy_ls.append(self._init_policy(n_s - n_w, n_a, n_a_p, n_a_s, n_w, n_f, model_config,
                                                    agent_name='{:d}a'.format(i)))
        self.saver = tf.train.Saver(max_to_keep=5)
        if total_step:
            self.total_step = total_step
