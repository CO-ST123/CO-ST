...
def parse_args():
    default_base_dir = 'small_net'
    default_config_dir = './config/config_ma2c_small_net.ini'
    ...
    parser.add_argument('--agents', type=str, required=False,
                        default='ma2c', help="agent folder names for evaluation, split by ,")
    parser.add_argument('--evaluation-policy-type', type=str, required=False, default='default',
...
def init_env(config, port=0, naive_policy=False):
    ...
    if config.get('scenario') == 'large_grid':
        if not naive_policy:
            ...
            return env, policy
    if config.get('scenario') == 'small_net':
        if not naive_policy:
            return LargeGridEnv(config, port=port)
        else:
            env = LargeGridEnv(config, port=port)
...
def train(args):
    ...
    if env.agent == 'ia2c':
        ...
        model = MA2C(env.n_s_ls, env.n_a_ls, env.n_a_lsp, env.n_a_lss, env.n_w_ls, env.n_f_ls, total_step,
                     config['MODEL_CONFIG'], seed=seed)
    elif env.agent == 'iqld':
...
def evaluate_fn(agent_dir, output_dir, seeds, port, demo, policy_type):
    ...
    if agent != 'greedy':
        if agent == 'a2c':
            ...
            model = MA2C(env.n_s_ls, env.n_a_ls, env.n_a_lsp, env.n_a_lss, env.n_w_ls, env.n_f_ls, 0, config['MODEL_CONFIG'])
        elif agent == 'iqld':
