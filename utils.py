...
class Trainer():
    ...
    def explore(self, prev_ob, prev_done):
        ...
        for _ in range(self.n_step):
            if self.agent.endswith('a2c'):
                ...
                if self.agent == 'a2c':
                    ...
                    action_s = []
                    for pi in policy:
                        action_p.append(np.random.choice(np.arange(len(pi[0:5])), p=np.array(pi[0:5])/sum(pi[0:5])))
                        action_s.append(np.random.choice(np.arange(len(pi[5:])), p=np.array(pi[5:])/sum(pi[5:])))

            else:
                action, policy = self.model.forward(ob, mode='explore')

            next_ob, reward, done, global_reward, global_first_two, global_last_one = self.env.step(action_p, action_s)

            rewards.append(global_reward)
            first_twos.append(global_first_two)
            last_ones.append(global_last_one)
            global_step = self.global_counter.next()
            self.cur_step += 1

            if self.agent.endswith('a2c'):
                self.model.add_transition(ob, action_p, action_s, reward, value, done)
            else:
                self.model.add_transition(ob, action_p, action_s, reward, next_ob, done)
        ...
        if self.agent.endswith('a2c'):
            ...
            R = 0
        return ob, done, R, rewards, first_twos, last_ones
    ...
    def perform(self, test_ind, demo=False, policy_type='default'):
        ...
        while True:
                ...
                if self.agent == 'a2c':
                    ...
                    for pi in policy:
                        if policy_type != 'deterministic':
                            ...
                            action_p.append(
                                np.random.choice(np.arange(len(pi[0:5])), p=np.array(pi[0:5]) / sum(pi[0:5])))
                            action_s.append(
                                np.random.choice(np.arange(len(pi[5:])), p=np.array(pi[5:]) / sum(pi[5:])))
                        else:
            ...
            # next_ob, reward, done, global_reward = self.env.step(action_p, action_s)
            next_ob, reward, done, global_reward, global_first_two, global_last_one = self.env.step(action_p, action_s)
            rewards.append(global_reward)
            first_twos.append(global_first_two)
            last_ones.append(global_last_one)
        ...
        mean_first_two = np.mean(np.array(first_twos))
        mean_last_one = np.mean(np.array(last_ones)) * 60
        std_reward = np.std(np.array(rewards))
        std_first_two = np.std(np.array(first_twos))
        std_last_one = np.std(np.array(last_ones))
        return std_reward, mean_first_two, std_first_two, mean_last_one, std_last_one
    ...
    def run(self):
        while not self.global_counter.should_stop():
            ...
            while True:
                ...
                ob, done, R, cur_rewards, cur_fist_two, cur_last_one = self.explore(ob, done)

                rewards += cur_rewards
                fist_two += cur_fist_two
                last_one += cur_last_one
                global_step = self.global_counter.cur_step
                if self.agent.endswith('a2c'):
                    self.model.backward(R, self.summary_writer, global_step)
                else:
            ...
            fist_two = np.array(fist_two)
            last_one = np.array(last_one)
            mean_reward = np.mean(rewards)
            mean_fist_two = np.mean(fist_two)
            mean_last_one = np.mean(last_one) * 60
            std_reward = np.std(rewards)
            std_fist_two = np.std(fist_two)
            std_last_one = np.std(last_one)
            log = {'agent': self.agent,
                   'step': global_step,
                   'test_id': -1,
                   # 'avg_reward': mean_reward,
                   'std_reward': std_reward,
                   'avg_fist_two': mean_fist_two,
                   'std_fist_two': std_fist_two,
                   'avg_last_one': mean_last_one,
                   'std_last_one': std_last_one
                   }

            logging.info('avg_fist_two: %.2f, avg_last_one: %.2f, std_reward: %.2f, std_fist_two: %.2f, std_last_one: %.2f,' % (mean_fist_two, mean_last_one, std_reward, std_fist_two, std_last_one))
...
class Evaluator(Tester):
    ...
    def run(self):
        ...
        time.sleep(1)
        for test_ind in range(self.test_num):
            _, first_two, _, last_one, _ = self.perform(test_ind, demo=self.demo, policy_type=self.policy_type)
            # std_reward, mean_first_two, std_first_two, mean_last_one, std_last_one
            self.env.terminate()
            logging.info('test %i, avg first_two %.2f, avg last_one %.2f' % (test_ind, first_two, last_one))
            time.sleep(2)
            self.env.collect_tripinfo()
        self.env.output_data()
